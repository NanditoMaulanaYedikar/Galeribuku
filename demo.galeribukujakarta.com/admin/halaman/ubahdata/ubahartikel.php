<?php
    include "koneksi.php";
    $id = $_GET['id'];
    $sql = "SELECT * FROM tb_artikel WHERE no_artikel = '$id'";
    $q = mysqli_query($k, $sql);
    $r = mysqli_fetch_assoc($q);

?>
<h3 class="mt-4">Form Artikel</h3>
<hr>
<style>
    .bold-button {
        padding: 5px 10px;
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="form-floating mb-3 mb-md-0">
                <input  name ="txtjudul" value="<?=$r['judul_artikel']?>" class="form-control" type="text" placeholder="Masukkan Judul" />
    ...
    <div class="form-floating mb-3">
        ...
        <textarea id="txtisi" name ="txtisi" class="form-control" rows="5"><?=$r['penjelasan_artikel']?></textarea>
    ...
    if (@$_POST['simpan']) {
        ...
        if (empty($gambar)) {
            $query = "UPDATE tb_artikel SET deskripsi='$deskripsi', judul_artikel='$judul', sinopsis='$sinopsis', penulis='$penulis', tanggal_terbit='$datetime', penjelasan_artikel='$isi', sponsor='$sponsor' WHERE no_artikel='$id'";
        } else {
            if (move_uploaded_file($tmp, "../img/$nama_gambar_baru")) {
                $query = "UPDATE tb_artikel SET gambar='$nama_gambar_baru', deskripsi='$deskripsi', judul_artikel='$judul', sinopsis='$sinopsis', penulis='$penulis', tanggal_terbit='$datetime', penjelasan_artikel='$isi', sponsor='$sponsor' WHERE no_artikel='$id'";
            } else {
