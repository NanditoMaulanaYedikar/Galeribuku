<?php
    include "koneksi.php";
    $id = $_GET['id'];
    $sql = "SELECT * FROM tb_jktplus WHERE no_jktplus = '$id'";
    $q = mysqli_query($k, $sql);
    $r = mysqli_fetch_assoc($q);


?>
<h3 class="mt-4">Form JKT+</h3>
<hr>
<style>
    .bold-button {
        padding: 5px 10px;
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    if (@$_POST['simpan']) {
        ...
        if (empty($gambar)) {
            $query = "UPDATE tb_jktplus SET judul='$judul', sinopsis='$sinopsis', penjelasan='$isi', deskripsi='$deskripsi', tanggal='$tanggal', penulis='$penulis', sponsor='$sponsor' WHERE no_jktplus='$id'";
        } else {
            if (move_uploaded_file($tmp, "../img/$nama_gambar_baru")) {
                $query = "UPDATE tb_jktplus SET  judul='$judul', sinopsis='$sinopsis', penjelasan='$isi', gambar='$nama_gambar_baru', deskripsi='$deskripsi', tanggal='$tanggal', penulis='$penulis', sponsor='$sponsor' WHERE no_jktplus='$id'";
            } else {
        ...
        if (mysqli_query($k, $query)) {
            echo "<script>alert('Data berhasil disimpan');location='.?hal=jktplus'</script>";
        } else {
