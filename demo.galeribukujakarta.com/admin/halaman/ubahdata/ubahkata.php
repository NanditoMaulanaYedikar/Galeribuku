...
include "koneksi.php";
$id = $_GET['id'];
$sql ="SELECT * FROM tb_kata WHERE no_kata = '$id'";
...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="form-floating mb-3 mb-md-0">
                ...
                <label for="inputFirstName">Judul Kata & Kota</label>
            </div>
        </div>
    </div>
    <div class="form-floating mb-3">
    ...
    if (@$_POST['simpan']) {
        ...
        if (empty($gambar)) {
            $query = "UPDATE tb_kata SET deskripsi='$deskripsi', judul='$judul', penulis='$penulis', penjelasan='$isi', tanggal='$datetime', sponsor='$sponsor' WHERE no_kata='$id'";
        } else {
            if (move_uploaded_file($tmp, "../img/$nama_gambar_baru")) {
                $query = "UPDATE tb_kata SET gambar='$nama_gambar_baru', deskripsi='$deskripsi', judul='$judul', penulis='$penulis', penjelasan='$isi', tanggal='$datetime', sponsor='$sponsor' WHERE no_kata='$id'";
            } else {
        ...
        if (mysqli_query($k, $query)) {
            echo "<script>alert('Data berhasil disimpan');location='.?hal=kata'</script>";
        } else {
