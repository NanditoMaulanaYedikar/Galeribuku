...
include "koneksi.php";
?>
<h3 class="mt-4">Form Kata & Kota</h3>
<hr>
<style>
    .bold-button {
        padding: 5px 10px;
    ...
    if (@$_POST['simpan']) {
        ...
        if (!empty($gambar) && is_uploaded_file($tmp)) {
            if (move_uploaded_file($tmp, "../img/$nama_gambar_baru")) {
                $query = "INSERT INTO tb_kata(gambar, deskripsi, judul, penulis, penjelasan, tanggal, sponsor) 
                        VALUES('$nama_gambar_baru', '$deskripsi', '$judul', '$penulis','$isi', '$datetime', '$sponsor')";
