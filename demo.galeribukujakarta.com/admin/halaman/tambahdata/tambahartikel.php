    ...
    if (@$_POST['simpan']) {
        ...
        if (!empty($gambar) && is_uploaded_file($tmp)) {
            if (move_uploaded_file($tmp, "../img/$nama_gambar_baru")) {
                $query = "INSERT INTO tb_artikel(gambar, deskripsi, judul_artikel, sinopsis, penulis, tanggal_terbit, penjelasan_artikel, sponsor) 
                        VALUES('$nama_gambar_baru', '$deskripsi', '$judul', '$sinopsis','$penulis', '$datetime', '$isi', '$sponsor')";
