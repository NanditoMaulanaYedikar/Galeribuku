...
<section id="artikel2">
    <div class="container">
    <?php
        $id1 = $_GET['no_pemikiran'];
        
        // Ambil nilai jumlah_dilihat dari database
        $sql_select = "SELECT jumlah_dilihat FROM tb_pemikiran WHERE no_pemikiran = '$id1'";
        ...
        if ($result) {
            ...
            $sql_update = "UPDATE tb_pemikiran SET jumlah_dilihat = $new_views WHERE no_pemikiran = '$id1'";
            mysqli_query($k, $sql_update);
        }

        $sql1 = "SELECT * FROM tb_pemikiran WHERE no_pemikiran='$id1' ORDER BY no_pemikiran DESC LIMIT 1";
...
<section id="readmore">
    <div class="container">
        <div class="slider">
        <?php
            $id1 = $_GET['no_pemikiran']; // Nomor artikel yang sedang ditampilkan

            // Query untuk mengambil artikel berdasarkan jumlah dilihat, kecuali yang sedang ditampilkan
            $sql_slider = "SELECT * FROM tb_pemikiran WHERE no_pemikiran != '$id1' ORDER BY jumlah_dilihat DESC";
            $q_slider = mysqli_query($k, $sql_slider);

            while ($r_slider = mysqli_fetch_assoc($q_slider)) {
            ?>
            <div class="slide">
                <a href="?page=pemikiran-detail&no_pemikiran=<?=$r_slider['no_pemikiran']?>">
