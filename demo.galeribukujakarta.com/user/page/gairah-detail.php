...
<section id="artikel1">
    <div class="container">
        <div class="head">
            ...
            $sql_select = "SELECT jumlah_dilihat FROM tb_gairah WHERE no_gairah = '$id1'";
            $result = mysqli_query($k, $sql_select);
            $row = mysqli_fetch_assoc($result);

            // Update nilai jumlah_dilihat
            $new_views = $row['jumlah_dilihat'] + 1;
            $sql_update = "UPDATE tb_gairah SET jumlah_dilihat = $new_views WHERE no_gairah = '$id1'";
            mysqli_query($k, $sql_update);
            
            $sql1 = "SELECT * FROM tb_gairah WHERE no_gairah='$id1' order by no_gairah desc limit 1";
...
<section id="readmore">
    <div class="container">
        <div class="slider">
        <?php
            $id2 = $_GET['no_gairah']; // Nomor artikel yang sedang ditampilkan

            // Query untuk mengambil artikel berdasarkan jumlah dilihat, kecuali yang sedang ditampilkan
            $sql_slider = "SELECT * FROM tb_gairah WHERE no_gairah != '$id2' ORDER BY jumlah_dilihat DESC";
            $q_slider = mysqli_query($k, $sql_slider);

            while ($r_slider = mysqli_fetch_assoc($q_slider)) {
            ?>
            <div class="slide">
                <a href="?page=gairah-detail&no_gairah=<?=$r_slider['no_gairah']?>">
