...
<section id="artikel2">
    <div class="container">
    <?php
        $id1 = $_GET['no_coffeesophia'];
        
        // Ambil nilai jumlah_dilihat dari database
        $sql_select = "SELECT jumlah_dilihat FROM tb_coffeesophia WHERE no_coffeesophia = '$id1'";
        ...
        if ($result) {
            ...
            $sql_update = "UPDATE tb_coffeesophia SET jumlah_dilihat = $new_views WHERE no_coffeesophia = '$id1'";
            mysqli_query($k, $sql_update);
        }

        $sql1 = "SELECT * FROM tb_coffeesophia WHERE no_coffeesophia='$id1' ORDER BY no_coffeesophia DESC LIMIT 1";
...
<section id="readmore">
    <div class="container">
        <div class="slider">
        <?php
            $id2 = $_GET['no_coffeesophia']; // Nomor artikel yang sedang ditampilkan

            // Query untuk mengambil artikel berdasarkan jumlah dilihat, kecuali yang sedang ditampilkan
            $sql_slider = "SELECT * FROM tb_coffeesophia WHERE no_coffeesophia != '$id2' ORDER BY jumlah_dilihat DESC";
            $q_slider = mysqli_query($k, $sql_slider);

            while ($r_slider = mysqli_fetch_assoc($q_slider)) {
            ?>
            <div class="slide">
                <a href="?page=coffeesophia-detail&no_coffeesophia=<?=$r_slider['no_coffeesophia']?>">
