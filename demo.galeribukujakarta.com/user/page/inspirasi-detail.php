...
<section id="artikel1">
    <div class="container">
        <div class="head">
            <?php
            $id1 = $_GET['no_inspirasi'];
            // Ambil nilai jumlah_dilihat dari database
            $sql_select = "SELECT jumlah_dilihat FROM tb_inspirasi WHERE no_inspirasi = '$id1'";
            $result = mysqli_query($k, $sql_select);
            $row = mysqli_fetch_assoc($result);

            // Update nilai jumlah_dilihat
            $new_views = $row['jumlah_dilihat'] + 1;
            $sql_update = "UPDATE tb_inspirasi SET jumlah_dilihat = $new_views WHERE no_inspirasi = '$id1'";
            mysqli_query($k, $sql_update);
            
            $sql1 = "SELECT * FROM tb_inspirasi WHERE no_inspirasi='$id1' order by no_inspirasi desc limit 1";
...
<section id="readmore">
    <div class="container">
        <div class="slider">
        <?php
            $id1 = $_GET['no_inspirasi']; // Nomor artikel yang sedang ditampilkan

            // Query untuk mengambil artikel berdasarkan jumlah dilihat, kecuali yang sedang ditampilkan
            $sql_slider = "SELECT * FROM tb_inspirasi WHERE no_inspirasi != '$id1' ORDER BY jumlah_dilihat DESC";
            $q_slider = mysqli_query($k, $sql_slider);

            while ($r_slider = mysqli_fetch_assoc($q_slider)) {
            ?>
            <div class="slide">
                <a href="?page=inspirasi-detail&no_inspirasi=<?=$r_slider['no_inspirasi']?>">
